...
int check_number(char *number)
{
    ...
    {
        printf("Введено неверное количество 'e'.\n");
        return UNRIGHT_NUMBER;
    }
    ...
    {
        printf("Неверная длина числа (m + n > 30).\n");
    ...
    {
        printf("Неверное расположение 'e' по отношению к '.'.\n");
        return UNRIGHT_NUMBER;
    }
    ...
    if (count_sign == 2) //Проверка на расположение + и - 
    {
        ...
        {
            if (position_sign(number, '+', '-') - position_e != 1)
            {
                printf("Неверное расположение знаков '+' или '-'.");
                return UNRIGHT_NUMBER;
            }
        }
        else 
    ...
    if (count_sign == 1) //
    {
        if ((number[0] != '-' && number[0] != '+'))
        {
            if (el_position(number, '-') < position_e || el_position(number, '-') < position_e)
            {
                printf("Неверное расположение знаков '+' или '-'.");
                return UNRIGHT_NUMBER;
            }
    ...
    if (count_sign >= 1) //Проверка порядка
    {
        ...
        {
            printf("Количество элементов в порядке больше 5.");
            return UNRIGHT_LEN_NUMBER;
        }
        else 
        {
